...
import { defaultFilesAllowlistPath, loadFilesAllowlist } from './registry.ts';
import {
  appendBlock,
  joinLines,
  readTextOrEmpty,
 ...
 * next one. Mirrors `setup-config-allowlist.ts`'s `findTable` (same TOML shape, different
 * file/field).
 */
...
/**
 * Appends a `[<alias>]` table to `files-allowlist.toml`, or, if `<alias>` already has a
 ...
 * is empty, checked before the `--yes` gate and before any file write. Gates every op in the
 * `files` group (`ls`/`cat`/`tail`/`download`/`disk-usage`/`upload`) — see
 * `registry.ts`'s `assertPathAllowed('files', ...)`.
 */
export function scaffold(
  alias: string,
  options: ScaffoldOptions,
  path: string = defaultFilesAllowlistPath(),
  ...
  const existingAllowlist = loadFilesAllowlist(path);
