...
### Phase 7: OSS packaging + trust signals + release pipeline — 2026-07-13

**Status:** Complete (audited ✅ — every trust claim traced to source and confirmed TRUE)
**Files created:** README.md, SECURITY.md, CONTRIBUTING.md, .github/workflows/{ci,release}.yml, .claude-plugin/{marketplace,plugin}.json, scripts/smoke.sh, scripts/sshd-fixture/{Dockerfile,docker-compose.yml,postgres-init/01-fixture.sql}
**Files modified:** docs/progress.md (prepended v0.1.0 session entry), docs/changelog.md (v0.1.0 user-facing entry), .gitignore (ignore smoke fixture ephemeral keydir)
**Key decisions:**
- README has 4 install channels (npx skills add / plugin marketplace / release binary w/ SHA-256 + provenance verify / just build), a "What sshepherd NEVER does" trust section, and a topics list. release.yml: 4 Bun target triples (darwin-arm64/x64, linux-x64/arm64) + per-binary .sha256 + actions/attest-build-provenance@v2 with id-token/attestations/contents write perms. ci.yml mirrors anywrite (bun run typecheck/lint/test/build — the sanctioned template pattern, not just-wrapped; CONTRIBUTING reworded to be mechanism-accurate). Plugin manifests valid JSON.
- SECURITY.md states the zero-knowledge invariant AND its honest boundary (sshepherd can't leak creds it never has, but ~/.ssh/config + ssh-agent security is the user's), the read-only-default/mutating-gated model, the db read-only layers, and the audit-log args_hash (never raw args).
- Smoke fixture (sshd + docker-cli + postgres w/ read-only role + nginx app) BUILT but NOT run live — Docker unavailable in the dev environment. Honestly marked as un-run in progress.md; the Linux command shapes remain doc-derived until a maintainer runs `just smoke` on a Docker host. NOT faked.
- dist/ confirmed gitignored + never staged. node-sql-parser remains the only runtime dep.
**Audit fixes (62f4cd3):** added README topics list (plan Phase 7 "Handles" line called for it); corrected CONTRIBUTING's "all three run in CI via just" to mechanism-accurate wording (CI uses bun run directly, matching anywrite).
**Test count:** 160 pass, `just check` clean, all manifests/workflows parse.
**User-driven follow-ups (NOT done autonomously — need the user's GitHub auth / a Docker host):** (1) create the github.com/Antheurus/sshepherd remote + push; (2) run `just smoke` on a Docker host to close the #1 shape-verification risk; (3) cut the v0.1.0 tag to trigger release.yml; (4) file awesome-list PRs after some stars; (5) verify npx skills add / plugin marketplace add resolve once pushed.

### Phase 7 audit — 2026-07-13
✅ All 5 success criteria + a full CLAIM-ACCURACY pass: every "What sshepherd NEVER does" / SECURITY.md claim traced to source and found TRUE — no key-material reads (only ~/.ssh/config alias tokens + remote authorized_keys as fingerprints), zero outbound calls (only Bun.spawn ssh; no fetch/http/telemetry anywhere in src/), no ad-hoc exec (recipe shell steps disclosed honestly as raw-shell-from-a-file-you-authored), audit hashes args (crc32, never raw), node-sql-parser the only runtime dep, 52 ops / 9 groups verified by counting REGISTRY, 12 mutating ops through one executeOp gate, db 3-layer read-only real. release.yml/ci.yml valid YAML + correct targets/perms; manifests valid JSON; no dist/ leak; smoke fixture honestly marked un-run; progress/changelog factually accurate. Two minor findings (CI uses bun run not just; README topics absent) → orchestrator fixed both (62f4cd3). 160 tests pass.

## Final status

**v1 COMPLETE (2026-07-13).** All 7 phases + the 5B scope-completion shipped and audited green. sshepherd is a compiled Bun/TS zero-knowledge SSH ops CLI + root SKILL.md: 9 command groups (hosts, check, logs, services, deploy, config, db, files, security), 52 ops, one transport core that shells out to system `ssh` and structurally cannot put a host/user/port/key into any Envelope, one confirm+audit gate for all 12 mutating ops, read-only Postgres introspection with 3-layer enforcement, TOML deploy recipes with dependency ordering + step-failure attribution, and the full OSS trust-signal set (README "NEVER does", SECURITY.md, no telemetry, 4-platform release w/ SHA-256 + build-provenance). node-sql-parser is the only runtime dependency. 160 unit tests pass (injectable fake ssh runner, zero real network); `just check` + `just test` green throughout. Two security defects were found by audit and fixed mid-build (db query COMMIT-injection bypass of the read-only txn; deploy step-failure attribution) — neither was ever a credential leak. ONE carried-forward verification gap: the Linux command output shapes + db introspection SQL are doc-derived, not yet run against a real Linux+docker+postgres host — the `just smoke` fixture is built and ready to close it, but Docker was unavailable in the build environment, so it must be run by the user (or on a Docker host) before the shape claims are considered live-verified. Repo is publishable but NOT pushed (user-driven: create remote, push, run smoke, cut v0.1.0 tag, awesome-list PRs).

**Cross-phase audit (od-finish, 2026-07-13):** APPROVE + regression GREEN. Zero-knowledge invariant verified holding end-to-end across all 9 groups + CLI + db/deploy resolvers — no leak path found (transportStderr read only inside classify() to pick a static code then discarded; Envelope has no host/user/port field to populate). All 12 mutating ops confirmed through the one executeOp gate. 52 ops, all plain OpSpec rows, no if/elif chains, shq/shellJoin on every interpolation. Zero Critical, zero Important. 160 tests pass, `just check`/`just build` green, binary driven live on all host-independent surfaces. One Minor (cli.ts closeMaster spawns an ssh -O exit even on the CONFIRMATION_REQUIRED refusal path that never opened a master — harmless, leak-free, optional guard).

**SMOKE RUN — LIVE VERIFIED (2026-07-13 cont):** Docker became available, `just smoke` ran **15/15 green** against the sshd+postgres+app fixture. Promotes every registry command shape + db introspection SQL from doc-derived (SHIPPED-UNVERIFIED) to VERIFIED-LIVE: hosts test, check overview (dead_end_risk/nproc), services ps (docker inspect), logs docker (LogsResult), files ls, security listeners (port 22), db list/tables/query (seeded row via read-only txn), deploy dry-run ordering. Fixture + ssh-config torn down clean. Remaining targeted check (auditor's Discussion item): `deploy status`/`services compose-ps` use `docker compose ps --format json` which can emit a single array vs NDJSON on newer compose — not in the smoke matrix, worth a spot-check. **PUBLISHED:** repo created (github.com/Antheurus/sshepherd, public MIT) + pushed; v0.1.0 tag cut to trigger release.yml.
