...
**Cross-phase audit (od-finish, 2026-07-13):** APPROVE + regression GREEN. Zero-knowledge invariant verified holding end-to-end across all 9 groups + CLI + db/deploy resolvers — no leak path found (transportStderr read only inside classify() to pick a static code then discarded; Envelope has no host/user/port field to populate). All 12 mutating ops confirmed through the one executeOp gate. 52 ops, all plain OpSpec rows, no if/elif chains, shq/shellJoin on every interpolation. Zero Critical, zero Important. 160 tests pass, `just check`/`just build` green, binary driven live on all host-independent surfaces. One Minor (cli.ts closeMaster spawns an ssh -O exit even on the CONFIRMATION_REQUIRED refusal path that never opened a master — harmless, leak-free, optional guard). **Smoke checklist item to verify FIRST when `just smoke` runs on a Docker host:** `docker compose ps --format json` on newer compose emits a single JSON array on one line, which `splitNdjson` would treat as one element — spot-check `deploy status`/`deploy logs`/`services compose-ps` shapes against real compose output.
