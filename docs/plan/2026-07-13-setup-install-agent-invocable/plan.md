...
### Phase 1 — done (commit 1233a3c, baseline 29e5c67)

Implemented `setup ssh-alias install` end-to-end: `src/setup-ssh-alias-install-server.ts` (new) — one-shot `Bun.serve()` on `127.0.0.1:0`, per-invocation random token gate (wrong/missing token → 404), 3-minute abandonment timeout via `Promise.race`, password piped to `sshpass -f /dev/stdin ssh ...` via stdin only (never env/disk/log — `buildSshpassArgs` structurally takes no password parameter), browser auto-open with print-URL fallback. `install(alias, options, configPath?, serverDeps?)` added to `src/setup-ssh-alias.ts`, reusing the managed stanza's `HostName`/`User`/`Port`/`IdentityFile` the same way `keygen`/`remove` already locate it. Wired as `ssh-alias`'s 4th action in `src/setup.ts` (help-text rewrite deliberately deferred to Phase 2). 3 new `SetupErrorCode`s added: `SSHPASS_NOT_FOUND`, `INSTALL_TIMED_OUT`, `INSTALL_FAILED` — the pre-keygen precondition (install attempted before a key exists) reuses `INSTALL_FAILED` with a distinct message rather than inventing a 4th code, since the spec capped new codes at 3; audited and confirmed reasonable.

Audited (second dispatch — first audit attempt stalled/timed out with no output, a clean infra retry, not a finding): ✅ all 8 success criteria spec-compliant, file inventory matches plan exactly. The password lifecycle was traced manually by the auditor in the diff itself (not taken on the executor's report) — `req.formData()` → local `password` var → `spawnInstall` → `stdin.write` → out of scope; `argsSummary` is `{}` at every `install`/register/keygen/remove call site so nothing password-shaped can reach `auditMutating`'s audit log. `bun test` re-run independently: 238 pass / 2 fail (the 2 failures are `skill-doc.test.ts`'s SKILL.md drift checks, expected until Phase 2 lands the docs rewrite — not a regression). `tsc --noEmit` and `biome check` both clean, re-run independently. 19 new tests read in full by the auditor, confirmed non-vacuous (real HTTP requests against the captured fetch handler, real timing races, an explicit negative assertion that the password string never appears in the returned outcome).

Criterion #5 (real live end-to-end drive: install onto a reachable box, verify passwordless `ssh <alias>` afterward) is **DEFERRED**, not performed. `sshpass` is confirmed installed on this machine, but no safe disposable SSH target exists — `nc -z localhost 22` confirms macOS Remote Login is off, no sshd Docker container is available, and the executor/auditor were both correctly instructed not to (a) use any of the user's 3 real production aliases (`module-brighty-prod`, `module-brighty-db`, `dev-server`) without explicit go-ahead, or (b) make any invasive system change (enabling Remote Login) without the user's consent. Closing this needs one of: user blesses a real alias for a one-time test, user authorizes enabling local Remote Login for a throwaway test, or user provides/spins up a disposable box. The install code path itself (`defaultSpawnInstall`, `buildRemoteInstallCommand`) is correct by inspection and mirrors `transport.ts`'s established spawn/timeout shape — it has just never been exercised against a real `sshpass`/`ssh` process yet.

### Phase 2 — done (commit 553dc8e, baseline 1233a3c)

Rewrote every stale "human-only, agent must never invoke" occurrence to the corrected model across `SKILL.md`, `README.md`, and `src/setup.ts`'s help text — located by grepping the current files for the phrasing itself rather than trusting the plan's (pre-Phase-1) line numbers. Touched: `SKILL.md`'s frontmatter description, the `--help` line, the Quick-reference heading (now "1 `setup` group (4 sub-groups, 7 actions)", was "1 human-only setup group ... 6 actions"), the Quick-reference block comment (also added the missing `sshepherd setup ssh-alias install myserver --yes` example line, which Phase 1 never added to the doc), Gotcha 9 (rewritten in place, not deleted — names all 7 actions as agent-invocable behind `--yes`, narrows the wall to the exact moment a human types a password into `install`'s browser form, and states the structural guarantee that the password goes straight into `sshpass`'s stdin and never crosses back into the agent's context). `README.md`'s `setup` paragraph and `src/setup.ts`'s help-text string were rewritten the same way (help-text change confined to the string literal — zero dispatch-logic lines touched, confirmed via diff). Gotcha 3's "human break-glass" reference to `plain ssh <alias>` was correctly left untouched — it's a genuinely different mechanism (manual raw SSH, not `setup`), not a missed occurrence.

Audited: ✅ spec compliant, zero stale "human-only"/"never invoke"/"off-limits" phrasing remaining anywhere in `SKILL.md`/`README.md`/`src/setup.ts` (confirmed via the auditor's own case-insensitive grep sweep, independent of the executor's location table). File scope held exactly to the expected 4 files — no implementation file touched. The drift-detection test's "real teeth" claim was independently re-verified by the auditor with two DIFFERENT mutations than the executor tried (removing the `keygen` Quick-reference line instead of `install`'s; changing the sub-group count instead of the action count) — both correctly caught the injected drift, both reverted cleanly, full suite confirmed green (240/240) after revert with zero residual diff. `bun test`/`tsc --noEmit`/`bun run lint` all re-run independently by the auditor, matching the executor's numbers exactly.

All Phase 1 + Phase 2 success criteria are met except Phase 1's criterion #5 (real live SSH drive), which remains DEFERRED pending user input on a safe test target — carried forward to `od-finish` as the one open item.
...
Both phases shipped and audited clean; cross-phase audit APPROVEd with GREEN regression (240/240 tests, tsc/lint clean) and zero Critical/Important findings — only one optional Minor (a slow-failure-not-a-leak hardening suggestion for `spawnInstall` throwing) and one Discussion note (a docs-wording nuance, no change needed), neither requiring action. `setup ssh-alias install` now exists as a genuinely new, fully agent-invocable action: the agent triggers it, a human types a password into a one-shot `127.0.0.1`-only browser form, and the password is proven — by three independent traces at increasing scope (Phase 1 audit, then cross-phase audit re-deriving from scratch) — to be structurally incapable of reaching the agent's context, a log, a file, or subprocess argv (`buildSshpassArgs` takes no password parameter at all). `SKILL.md`/`README.md`/`setup.ts`'s help text now accurately describe the corrected model: all 7 `setup` actions (`register`/`keygen`/`install`/`remove`/`db-target`/`config-allowlist`/`deploy-recipe`) are agent-invocable behind the same `--yes` gate as every other mutating action; the only wall is the literal password-entry moment inside `install`'s form. This build required zero changes to `register`/`keygen`/`remove`/the 3 scaffolders' actual logic, confirming the original miscommunication (the first build's "human-only" framing) was a documentation-only problem, not an architectural one — corrected in 2 phases rather than a revert-and-redo.

**Criterion #5 closed post-finish (commit `30cf3be`), with a real bug caught in the process.** The user chose to bless `dev-server` for the live drive; since that alias predates `setup` and has no sshepherd-managed marker, a parallel throwaway alias (`dev-server-sshepherd-livetest`) was registered against the same box instead, to avoid touching the working config. The FIRST live attempt failed: the browser navigated to `127.0.0.1:PORT/submit` (404) instead of `/TOKEN/submit` — `renderFormHtml`'s `<form action="submit">` was a relative URL, which resolves against a page served at `/TOKEN` (no trailing slash) by replacing the last path segment, dropping the token entirely per standard browser URL-resolution rules. Three independent code-reading passes (Phase 1 executor, Phase 1 auditor, cross-phase auditor) all missed this because every unit test constructs its `POST` request with the correct `/TOKEN/submit` path directly — none of them render the HTML and resolve the `action` attribute the way a real browser does. Fixed to an absolute `/${token}/submit` action; added a regression test that parses the rendered form's actual `action` attribute and resolves it via `new URL(action, pageUrl)`, the same mechanism a browser uses, so this exact bug class can't silently reappear. Rebuilt, re-ran end to end: `install` returned `ok:true`, and `ssh dev-server-sshepherd-livetest` (BatchMode, no agent/key prompt) succeeded — **criterion #5 is VERIFIED-LIVE**, not DEFERRED. Cleaned up fully afterward: the test public key was stripped from the remote's `authorized_keys` (manual, since `remove` only cleans local `~/.ssh/config`/local key files by design, never touches a remote), the local alias/key removed via `setup ssh-alias remove`, and the original `dev-server` alias confirmed still passwordless-working and untouched. Per the same live-drive session, the one-shot form/success/error pages also got a hand-rolled dark/terminal styling (inline CSS only — no external stylesheet/script, since this page is where a password is typed and the tool's whole design point is zero external dependencies).

PR #1 (still open against `main`) now carries both this build's commits and the first build's — an amendment to the same PR, not a new one. Binary at `~/.local/bin/sshepherd` rebuilt and current as of `30cf3be`.
