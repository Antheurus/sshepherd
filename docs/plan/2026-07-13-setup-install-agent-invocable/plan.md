...
The one item carried forward DEFERRED, not silently skipped: success criterion #5, a real live end-to-end install-and-verify drive against a reachable SSH box. No safe disposable target exists on the build machine (macOS Remote Login off, no local sshd Docker container), and the executor/auditors were correctly instructed not to touch the user's 3 real registered production aliases or make any invasive system change without explicit consent. The install code path is verified correct by structural inspection and mirrors the project's existing `transport.ts` spawn/timeout pattern, but has never been exercised against a real `sshpass`/`ssh` process. Closing this needs the user to either bless a real alias for a one-time test, authorize enabling local Remote Login for a throwaway test, or provide/spin up a disposable box.

PR #1 (still open against `main`) now carries both this build's commits and the first build's — an amendment to the same PR, not a new one. Binary at `~/.local/bin/sshepherd` was stale as of Phase 1's start (predates `install`); rebuild before any live manual drive.
