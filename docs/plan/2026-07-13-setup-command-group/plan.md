...
### Cross-phase audit — 2026-07-13

**Reviewer:** orchestration-cross-phase-auditor (Opus, one-shot, dispatched by od-finish after all 6 phases + 2 insertions were individually ✅)
**Date:** 2026-07-13

**Verdict:** NEEDS-FIXES (initial pass) → fixes applied → independently re-verified by the orchestrator → APPROVE

A fresh, independent read of the full `41f3535..0122ab1` diff found what the 6 per-phase in-loop audits, each scoped to its own phase's diff, structurally could not see: a real cross-cutting bug only visible once all 4 scaffolders existed side by side.

**Critical (blocking):** TOML string values were never escaped in any of the 3 scaffolders (`setup-db-target.ts`, `setup-config-allowlist.ts`, `setup-deploy-recipe.ts`) — a value containing `"` (e.g. `--alias 'my"alias'`) produced invalid TOML that crashed the *next* `db`/`deploy` command with an unhandled Bun exception and raw stack trace, not the graceful `SetupResult`/`Envelope` error this codebase promises everywhere else. Reproduced live by the auditor, then independently re-reproduced by the orchestrator both before and after the fix.

**Important (non-blocking but fixed):** `splitLines`/`joinLines` duplicated byte-for-byte between `setup-ssh-alias.ts` and `setup-config-allowlist.ts` (the latter's own comment admitted mirroring the former); `runStubAction` in `setup.ts` was dead code once all 4 real sub-groups were wired (unreachable fallthrough); no automated test exercised the full register→keygen→db-target→config-allowlist→deploy-recipe→remove chain, only each sub-command in isolation; `docs/progress.md` was never updated across any of the 8 phase commits, despite `CONTRIBUTING.md` requiring an entry.

**Minor (fixed):** `setup ssh-alias` had no env-var path override, unlike the other 3 sub-commands, forcing any CLI-level test to touch the real `~/.ssh/config`.

**Fix cycle (single pass, per od-finish's one-shot rule):** all 6 findings fixed in commit `e2edc07`. Notably, the fix-implementer discovered a real Bun v1.3.6 bug while choosing an escaping strategy — `JSON.stringify`'s `\t` escape is misparsed by `Bun.TOML.parse` as `\f` — and hand-rolled `src/toml-quote.ts` instead of relying on it, verified via direct round-trip against the real `Bun.TOML.parse`. `bun test` grew from 212 to 221 (new: TOML-injection regression test, a dedicated `toml-quote.test.ts`, and `setup-integration.test.ts` covering the full cross-sub-command chain), zero pre-existing tests changed. `tsc --noEmit`/`biome check` both clean.

**Orchestrator's independent re-verification (not delegated):** re-ran `bun test` (221/221), `tsc --noEmit`, `bun run lint` directly; personally re-reproduced the exact Critical-1 repro command (`setup db-target scaffold weird --alias 'my"alias' ...`) and confirmed the file now writes correctly-escaped TOML and the subsequent `db list` returns a graceful `ok:true` instead of crashing — verified live, not trusted from the fix-implementer's report alone.

## Final status

Built the `sshepherd setup` command group end to end across 6 planned phases plus 2 orchestrator-inserted consolidation steps (triggered by audit findings, not pre-planned): a fully separate, non-registry dispatch path (`src/setup.ts`, `src/setup-types.ts`) intercepted in `cli.ts` before the existing `GROUPS` check; `setup ssh-alias register/keygen/remove` writing marked stanzas to `~/.ssh/config` with dedicated per-alias ed25519 keys; `setup db-target`/`config-allowlist`/`deploy-recipe` scaffolding the three remaining config surfaces, each round-tripped through its real existing parser/validator; and full `SKILL.md`/`README.md`/`docs/changelog.md` documentation with a drift-detection test proven (via live mutation) to actually catch future doc/code divergence. Scope was correctly narrowed mid-research (no remote key install, no interactive password/browser flow needed) after the user's own clarification, which eliminated what would have been the single largest source of implementation risk. The build closes the exact incident that motivated it: an AI agent can no longer be the only path to registering a new SSH alias for this tool. A subsequent cross-phase audit — reading all 8 commits together for the first time, something no single per-phase audit could do — found one real, reproducible crash bug (unescaped TOML string values) plus 4 legitimate cleanup items; all 5 were fixed in one pass and independently re-verified by the orchestrator, not just trusted from the fix report. Final state: 221 tests green (up from 165 at Phase 1, zero regressions at any phase boundary), `tsc`/`biome` clean throughout, every mutating success criterion verified live against real files and real parsers rather than mocks. One process deviation was caught and corrected mid-run: Phases 1-3 had been committed directly to `main` in violation of the standing "never work straight on main" rule; fixed by moving all work to `feat/setup-command-group` (pushed to `origin`) and resetting local `main` back to `origin/main`'s tip before continuing — safe since nothing had been pushed yet. Two non-blocking follow-ups remain open (see Review findings, pre-fix section) and are not blockers to shipping: `appendBlock`'s whitespace-normalization edge case, and `.claude-plugin/plugin.json`'s unbumped version.
